...
class RolloutTrainerMixin(RLHFTrainerMixin):
    ...
    def _colocate_multi_turn_infer(self, inputs: DataType, first_turn_rollout_outputs: List[RolloutOutput],
        ...
        while True:
            ...
            for stop, index, output in zip(should_stops, index_to_infer, outputs):
                ...
                if stop:
                    ...
                    if is_continuation and response_token_ids[index]:
                        ...
                        if response_loss_mask[index]:
                            ...
                            # 与本轮 token 对齐：调度器若未返回 mask，默认整段参与 loss
                            response_loss_mask[index] = [[1] * len(response_token_ids[index][-1])]
                        ...
                        if final_token_ids:
                            ...
                            # 中间轮若只写了 response_token_ids、未写 mask，此处先为前几轮补全与 token 等长的全 1 mask
                            while len(response_loss_mask[index]) < len(response_token_ids[index]) - 1:
                                ti = len(response_loss_mask[index])
                                response_loss_mask[index].append([1] * len(response_token_ids[index][ti]))
                ...
                elif 'response_token_ids' in step_result:
                    # 调度器只返回 token、未返回 mask 时，与 response_token_ids 同结构默认全 1（与 replace_assistant_response_with_ids 对齐）
                    n = len(step_result['response_token_ids'])
                    default = [1] * n
                    if is_continuation and response_token_ids[index]:
                        if response_loss_mask[index]:
                            response_loss_mask[index][-1].extend(default)
                        else:
                            response_loss_mask[index] = [[1] * len(response_token_ids[index][-1])]
                    else:
                        response_loss_mask[index].append(default)
    ...
    def _preprocess_inputs(self, inputs: DataType) -> DataType:
        ...
        for input_item in processed_inputs:
            preprocess = (
                getattr(self.multi_turn_scheduler, 'preprocess_rollout_messages', None)
                if self.multi_turn_scheduler else None)
            if callable(preprocess):
                preprocess(input_item)
            else:
