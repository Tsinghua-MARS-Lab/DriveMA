...
class GRPOConfig(GRPOArgumentsMixin, TrainArgumentsMixin, HfGRPOConfig):
    ...
    def __post_init__(self):
        ...
        if getattr(self, 'per_turn_grpo_credit', False) and getattr(self, 'use_liger_kernel', False):
            raise ValueError('per_turn_grpo_credit 与 use_liger_kernel 不能同时开启（Liger GRPO 仅支持标量 advantage）。')
