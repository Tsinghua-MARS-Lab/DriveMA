...
from swift.template import Template, TemplateInputs, get_last_user_round
...
class GRPOTrainer(RolloutTrainerMixin, SwiftMixin, HFGRPOTrainer):
    ...
    def _generate_and_score_completions(self, inputs: DataType) -> DataType:
        ...
        if getattr(self.args, 'per_turn_grpo_credit', False):
            from .turn_credit import extract_two_assistant_messages
            for inp in inputs:
                g1, g2 = extract_two_assistant_messages(inp['messages'])
                inp['_logged_completion_turn1'] = g1
                inp['_logged_completion_turn2'] = g2
        ...
        if getattr(self.args, 'per_turn_grpo_credit', False):
            local_advantages = total_advantages
        else:
        ...
        if isinstance(total_advantages, list):
            advantage_logs = [adv.float().mean().item() for adv in total_advantages]
            self._logs['advantages'].extend(self._gather_and_flatten(advantage_logs, flatten_level=0))
        elif total_advantages.dim() == 2:
            self._logs['advantages'].extend(total_advantages.mean(dim=-1).tolist())
        else:
        ...
        for batch, batch_encoded in zip(gas_chunks, batch_encoded_inputs):
            # Scalar advantages are broadcast in loss; per-turn credit stores token-level rows directly.
        ...
        with profiling_context(self, 'log_metrics'):
            ...
            completion_turn1 = [inp.get('_logged_completion_turn1', '') for inp in inputs]
            completion_turn2 = [
                inp.get('_logged_completion_turn2', '') or completion
            ...
            valid_completion_turn1 = self._gather_and_flatten(completion_turn1, flatten_level=0)
            valid_completion_turn2 = self._gather_and_flatten(completion_turn2, flatten_level=0)
            ...
            self._logs['completion_turn1'].extend(valid_completion_turn1)
            self._logs['completion_turn2'].extend(valid_completion_turn2)
    ...
    def _compute_per_turn_rewards_per_func(self, inputs: DataType) -> torch.Tensor:
        """Compute turn-level rewards and store _grpo_r1/_grpo_r2 for token-level credit assignment."""
        from .turn_credit import extract_two_assistant_messages, get_gt_assistant_turns
        device = self.accelerator.device
        n = len(inputs)
        names = self.reward_func_names
        first_turn_reward_names = {'turn1_format'}
        second_turn_reward_names = {
            'ade_5s', 'ade_4s', 'rfs', 'pdms', 'traj_consistency', 'traj_format', 'turn2_format', 'traj_length'
        }
        if 'meta_action_acc' not in names:
            raise ValueError('per_turn_grpo_credit 需要 reward_funcs 包含 meta_action_acc')
        if not (set(names) & second_turn_reward_names):
            raise ValueError(f'per_turn_grpo_credit 需要至少一个第二轮 reward: {second_turn_reward_names}')
        unsupported = [
            name for name in names
            if name != 'meta_action_acc' and name not in first_turn_reward_names and name not in second_turn_reward_names
        ]
        if unsupported:
            raise ValueError(f'per_turn_grpo_credit 暂不支持这些 reward_funcs: {unsupported}')
        if self._async_reward_func_indices:
            raise ValueError('per_turn_grpo_credit 不支持异步 reward')
        if any(isinstance(f, nn.Module) for f in self.reward_funcs):
            raise ValueError('per_turn_grpo_credit 不支持 reward_model 形式的 nn.Module')
        rewards_per_func = torch.zeros((n, len(self.reward_funcs)), device=device)
        im = names.index('meta_action_acc')
        first_turn_indices = [i for i, name in enumerate(names) if name in first_turn_reward_names]
        second_turn_indices = [i for i, name in enumerate(names) if name in second_turn_reward_names]
        gen1s, gen2s, sol1s, sol2s = [], [], [], []
        for inp in inputs:
            g1, g2 = extract_two_assistant_messages(inp['messages'])
            t1, t2 = get_gt_assistant_turns(inp)
            gen1s.append(g1)
            gen2s.append(g2)
            sol1s.append(t1)
            sol2s.append(t2)
        reward_inputs = [{k: v for k, v in inp.items() if k != 'add_eos'} for inp in inputs]
        rk = {'trainer_state': self.state}
        rk.update(RowPreprocessor.rows_to_batched(reward_inputs))
        rk.pop('solution', None)
        meta_out = self.reward_funcs[im](gen1s, solution=sol1s, **rk)
        reward_outputs = {im: meta_out}
        rewards_per_func[:, im] = torch.tensor(meta_out, dtype=torch.float32, device=device)

        for idx in first_turn_indices:
            func = self.reward_funcs[idx]
            out = func(gen1s, **rk)
            reward_outputs[idx] = out
            rewards_per_func[:, idx] = torch.tensor(out, dtype=torch.float32, device=device)

        for idx in second_turn_indices:
            name = names[idx]
            func = self.reward_funcs[idx]
            if name in {'ade_5s', 'ade_4s'}:
                out = func(gen2s, solution=sol2s, **rk)
            elif name == 'traj_consistency':
                rk_cons = dict(rk)
                rk_cons['per_turn_credit_mode'] = True
                out = func([''] * n, **rk_cons)
            else:
                # rfs / pdms / traj_format / traj_length are trajectory-output rewards, so credit them to turn 2.
                out = func(gen2s, **rk)
            reward_outputs[idx] = out
            rewards_per_func[:, idx] = torch.tensor(out, dtype=torch.float32, device=device)
        ...
        for i, inp in enumerate(inputs):
            # 第一轮标量 = meta action + 第一轮格式类 reward
            inp['_grpo_r1'] = float(meta_out[i]) + sum(
                float(w[idx].item()) * float(reward_outputs[idx][i]) for idx in first_turn_indices)
            # 第二轮标量 = sum(weight_i * reward_i) for trajectory-output rewards
            inp['_grpo_r2'] = sum(
                float(w[idx].item()) * float(reward_outputs[idx][i]) for idx in second_turn_indices)
        return rewards_per_func

    def _compute_rewards_per_func(self, inputs: DataType) -> torch.Tensor:
        """Compute rewards using all reward functions"""
        if getattr(self.args, 'per_turn_grpo_credit', False):
            return self._compute_per_turn_rewards_per_func(inputs)
        device = self.accelerator.device
    ...
    def _group_scalar_advantage(self, r: torch.Tensor, K: int,
                                normalize_advantages) -> torch.Tensor:
        """Group GRPO/RLOO/REINFORCE++ scalar advantages on a tensor (same layout as default path)."""
        grouped = r.view(-1, K)
        group_rewards_mean = grouped.mean(dim=1).repeat_interleave(K)
        if self.advantage_estimator == 'rloo':
            if K > 1:
                advantages = r * K / (K - 1) - group_rewards_mean * K / (K - 1)
            else:
                advantages = r - group_rewards_mean
        else:
            advantages = r - group_rewards_mean
        if self.advantage_estimator == 'reinforce_plus_plus':
            ...
            if self.scale_rewards == 'batch':
                if r.numel() > 1:
                    rewards_std = r.std().expand_as(r)
                else:
                    rewards_std = torch.zeros_like(r)
            elif self.scale_rewards == 'group':
                if K > 1:
                    rewards_std = grouped.std(dim=1).repeat_interleave(K)
                else:
                    rewards_std = torch.zeros_like(r)
            else:
                rewards_std = None
    ...
    def _compute_per_turn_advantages(self, inputs: DataType, rewards_per_func: torch.Tensor,
                                     batch_encoded_inputs: List[DataType]) -> List[torch.Tensor]:
        """第一轮/第二轮标量 reward 分别做全局组内 GRPO，再映射到本地对应轮 token。"""
        device = self.accelerator.device
        mode = 'train' if self.model.training else 'eval'
        ...
        r1 = torch.tensor([float(inp.get('_grpo_r1', 0.0)) for inp in inputs], device=device, dtype=torch.float32)
        r2 = torch.tensor([float(inp.get('_grpo_r2', 0.0)) for inp in inputs], device=device, dtype=torch.float32)
        ...
        # 标量 reward 先按全局 batch 做组内 GRPO；token mask 保持本地，避免不同 rank 的 logits_to_keep 不同。
        r1 = gather(r1)
        r2 = gather(r2)
        if kl is not None:
            r1 = r1 - self.beta * kl
        ...
        adv1 = self._group_scalar_advantage(r1, K, normalize_advantages)
        adv2 = self._group_scalar_advantage(r2, K, normalize_advantages)
        local_adv1 = get_even_process_data(self, adv1)
        local_adv2 = get_even_process_data(self, adv2)

        local_rows: List[torch.Tensor] = []
        offset = 0
        for be in batch_encoded_inputs:
            if 'turn1_completion_mask' not in be:
                raise ValueError(
                    'per_turn_grpo_credit 需要两轮的 response_token_ids 与 turn_mask；'
                    '请确认 multi_turn rollout 与 _prepare_batch_inputs 已写入 turn*_completion_mask')
            m1 = be['turn1_completion_mask']
            m2 = be['turn2_completion_mask']
            chunk_size = m1.shape[0]
            chunk_adv1 = local_adv1[offset:offset + chunk_size].to(device=m1.device)
            chunk_adv2 = local_adv2[offset:offset + chunk_size].to(device=m2.device)
            if chunk_adv1.shape[0] != chunk_size or chunk_adv2.shape[0] != chunk_size:
                raise ValueError('本地 turn mask batch 与 advantage 长度不一致')
            rows = chunk_adv1.unsqueeze(1) * m1.float() + chunk_adv2.unsqueeze(1) * m2.float()
            local_rows.extend([row for row in rows])
            offset += chunk_size
        if offset != len(inputs):
            raise ValueError('本地 turn mask 数量与 inputs 数量不一致')

        weighted = (rewards_per_func * self.reward_weights.unsqueeze(0)).nansum(dim=1)
        grouped_rewards = weighted.view(-1, num_generations)
        ...
        if self.scale_rewards in ['group', 'none', 'gdpo']:
            ...
            rewards_std = weighted.std().item() if weighted.numel() > 1 else 0.0
        if num_generations > 1:
            is_std_zero = torch.isclose(grouped_rewards.std(dim=1), torch.zeros_like(grouped_rewards.std(dim=1)))
        else:
            is_std_zero = torch.ones(grouped_rewards.size(0), dtype=torch.bool, device=grouped_rewards.device)
    ...
    def _compute_advantages(self, inputs: DataType, rewards_per_func: torch.Tensor,
                            batch_encoded_inputs: List[DataType]) -> Union[torch.Tensor, List[torch.Tensor]]:
        """
        ...
        if getattr(self.args, 'per_turn_grpo_credit', False):
            if self.dynamic_num_samples:
                raise ValueError('per_turn_grpo_credit 暂不支持 dynamic_num_samples')
            if self.scale_rewards == 'gdpo':
                raise ValueError('per_turn_grpo_credit 暂不支持 scale_rewards=gdpo')
            return self._compute_per_turn_advantages(inputs, rewards_per_func, batch_encoded_inputs)
    ...
    def _get_prefix_loss_mask(self, template: Template, prefix: str, query: str) -> Tuple[List[int], List[int]]:
        contexts, scales = template.loss_scale.get_loss_scale(prefix, query=query)
        token_ids: List[int] = []
        loss_mask: List[int] = []
        for context, scale in zip(contexts, scales):
            ids = template._tokenize(context) if isinstance(context, str) else context
            token_ids.extend(ids)
            loss_mask.extend([1 if scale > 0.0 else 0] * len(ids))
        return token_ids, loss_mask

    def _apply_non_thinking_prefix_to_response_ids(self, data: Dict[str, Any], template: Template) -> None:
        """Preserve add_non_thinking_prefix/loss_scale behavior when assistant content is replaced by token ids."""
        if not getattr(template, 'use_chat_template', False) or not getattr(template, 'add_non_thinking_prefix', False):
            return
        prefix = template.template_meta.non_thinking_prefix
        if not prefix:
            return
        messages = data.get('messages') or []
        response_token_ids = data.get('response_token_ids')
        if not response_token_ids or not isinstance(response_token_ids, list):
            return
        if not template.is_training or template.loss_scale.base_strategy == 'last_round':
            start_idx = get_last_user_round(messages)
        else:
            start_idx = -1
        loss_mask = data.get('response_loss_mask')
        if loss_mask is None or not isinstance(loss_mask, list) or len(loss_mask) != len(response_token_ids):
            loss_mask = [[1] * len(ids) for ids in response_token_ids]
            data['response_loss_mask'] = loss_mask
        assistant_turn = 0
        for i, message in enumerate(messages):
            if message.get('role') != 'assistant':
                continue
            if assistant_turn >= len(response_token_ids):
                break
            content = message.get('content')
            if (template._is_add_non_thinking_round(messages, i, start_idx) and isinstance(content, str)
                    and not content.startswith(('<think>', prefix))):
                query = messages[i - 1].get('content', '') if i > 0 and messages[i - 1].get('role') == 'user' else ''
                prefix_ids, prefix_mask = self._get_prefix_loss_mask(template, prefix, query)
                response_token_ids[assistant_turn] = prefix_ids + list(response_token_ids[assistant_turn])
                loss_mask[assistant_turn] = prefix_mask + list(loss_mask[assistant_turn])
                message['content'] = prefix + content
            assistant_turn += 1

    def _build_turn_completion_masks(self, batch: DataType, completion_mask: torch.Tensor):
        """按两轮有效 loss token 数，将 completion_mask 中为 True 的 token 依次划给第一轮/第二轮。"""
        t1_rows = []
        t2_rows = []
        for bi, data in enumerate(batch):
            rtids = data.get('response_token_ids')
            if not rtids or not isinstance(rtids, (list, tuple)) or len(rtids) < 2:
                return None, None
            if len(rtids) != 2:
                raise ValueError('per_turn_grpo_credit 仅支持恰好两轮 assistant response_token_ids')
            loss_mask = data.get('response_loss_mask')
            if loss_mask and isinstance(loss_mask, (list, tuple)) and len(loss_mask) >= 2:
                n1 = int(sum(loss_mask[0])) if isinstance(loss_mask[0], (list, tuple)) else 0
                n2 = int(sum(loss_mask[1])) if isinstance(loss_mask[1], (list, tuple)) else 0
            else:
                n1 = len(rtids[0]) if isinstance(rtids[0], (list, tuple)) else 0
                n2 = len(rtids[1]) if isinstance(rtids[1], (list, tuple)) else 0
            idx = completion_mask[bi].nonzero(as_tuple=True)[0]
            m1 = torch.zeros_like(completion_mask[bi])
            m2 = torch.zeros_like(completion_mask[bi])
            if len(idx) >= n1 + n2:
                m1[idx[:n1]] = True
                m2[idx[n1:n1 + n2]] = True
            elif len(idx) >= n1:
                m1[idx[:n1]] = True
                m2[idx[n1:]] = True
            else:
            ...
            t1_rows.append(m1)
            t2_rows.append(m2)
        return torch.stack(t1_rows), torch.stack(t2_rows)
    ...
    def _prepare_batch_inputs(self, inputs: DataType) -> List[DataType]:
        ...
        for batch in gas_chunks:
            ...
            with self._template_context(template):
                for data in batch:
                    if 'response_token_ids' in data and data['response_token_ids']:
                        self._apply_non_thinking_prefix_to_response_ids(data, template)
                        loss_mask = None
            ...
            if self.template.padding_free:
                ...
                # In non-padding-free mode, completion_mask is already [batch_size, logits_to_keep]
                completion_mask = completion_mask_raw

            extra_kwargs['completion_mask'] = completion_mask
            if getattr(self.args, 'per_turn_grpo_credit', False):
                t1m, t2m = self._build_turn_completion_masks(batch, completion_mask)
                if t1m is not None:
                    extra_kwargs['turn1_completion_mask'] = t1m
                    extra_kwargs['turn2_completion_mask'] = t2m
    ...
    def _compute_loss_and_metrics(self, model, inputs):
        ...
        if self.off_policy_sequence_mask_delta is not None:
            ...
            if advantages.dim() == 2:
                adv_for_mask = (advantages * completion_mask).sum(-1) / completion_mask.sum(-1).clamp(min=1.0)
            else:
        ...
        def compute_turnwise_mean_loss() -> Optional[torch.Tensor]:
            if not getattr(self.args, 'per_turn_grpo_credit', False):
                return None
            turn1_mask = inputs.get('turn1_completion_mask')
            turn2_mask = inputs.get('turn2_completion_mask')
            if turn1_mask is None or turn2_mask is None:
                return None

            turn1_mask = turn1_mask.to(device=completion_mask.device) & completion_mask
            turn2_mask = turn2_mask.to(device=completion_mask.device) & completion_mask

            w1 = float(self.args.per_turn_loss_weight_turn1)
            w2 = float(self.args.per_turn_loss_weight_turn2)
            per_sample_loss = torch.zeros(
                completion_mask.shape[0], dtype=per_token_loss.dtype, device=completion_mask.device)
            per_sample_weight = torch.zeros_like(per_sample_loss)

            for turn_mask, turn_weight in ((turn1_mask, w1), (turn2_mask, w2)):
                if turn_weight <= 0:
                    continue
                turn_token_count = turn_mask.sum(-1)
                valid_turn = turn_token_count > 0
                if valid_turn.any():
                    turn_loss = (per_token_loss * turn_mask).sum(-1) / turn_token_count.clamp(min=1.0)
                    per_sample_loss = per_sample_loss + turn_loss * (valid_turn.float() * turn_weight)
                    per_sample_weight = per_sample_weight + valid_turn.float() * turn_weight

            valid_sample = per_sample_weight > 0
            if not valid_sample.any():
                return None
            return (per_sample_loss[valid_sample] / per_sample_weight[valid_sample]).mean()

        if self.loss_type in ['grpo', 'sapo']:
            # completion_mask is now always [batch_size, seq_len] after pad_back
            loss = compute_turnwise_mean_loss()
            if loss is None:
        ...
        if self.loss_type == 'cispo':
            ...
            is_cispo_clipped = (coef_1 > self.epsilon_high) & (advantages_expanded > 0)
    ...
    def _prepare_metrics(self):
        ...
        self._logs = {
            ...
            'completion_turn1': deque(maxlen=args.generation_batch_size),
            'completion_turn2': deque(maxlen=args.generation_batch_size),
    ...
    def _prepare_rewards(self, reward_funcs, reward_model=None, reward_templates=None):
        ...
        # 从 orms 按名字实例化时，reward_func_names 必须保留注册名（如 meta_action_acc），
        # 不能用类名（MetaActionAccReward），否则 per_turn_grpo_credit 等按名索引会失败。
        ...
        if reward_funcs:
            for i, reward_func in enumerate(reward_funcs):
                if reward_func in orms:
                    ...
                    self.reward_func_names.append(reward_func)
