...
class GRPOTrainer(RolloutTrainerMixin, SwiftMixin, HFGRPOTrainer):
    ...
    def _compute_per_turn_rewards_per_func(self, inputs: DataType) -> torch.Tensor:
        ...
        second_turn_reward_names = {
            'ade_5s', 'rfs', 'pdms', 'traj_consistency', 'traj_format', 'turn2_format', 'traj_length'
        }
    ...
    def _compute_per_turn_advantages(self, inputs: DataType, rewards_per_func: torch.Tensor,
        ...
        """Sum turn rewards first, then assign the same normalized GRPO advantage to all completion tokens."""
        device = self.accelerator.device
        mode = 'train' if self.model.training else 'eval'
        ...
        total_rewards = torch.tensor([
            float(inp.get('_grpo_r1', 0.0)) + float(inp.get('_grpo_r2', 0.0)) for inp in inputs
        ],
                                     device=device,
                                     dtype=torch.float32)
        ...
        # 标量 reward 先按全局 batch 做一次原始 GRPO；token mask 保持本地，避免不同 rank 的 logits_to_keep 不同。
        total_rewards = gather(total_rewards)
        if kl is not None:
            total_rewards = total_rewards - self.beta * kl

        advantages = self._group_scalar_advantage(total_rewards, K, normalize_advantages)
        local_advantages = get_even_process_data(self, advantages)

        local_rows: List[torch.Tensor] = []
        offset = 0
        for be in batch_encoded_inputs:
            completion_mask = be['completion_mask']
            chunk_size = completion_mask.shape[0]
            chunk_advantages = local_advantages[offset:offset + chunk_size].to(device=completion_mask.device)
            if chunk_advantages.shape[0] != chunk_size:
                raise ValueError('本地 completion mask batch 与 advantage 长度不一致')
            rows = chunk_advantages.unsqueeze(1) * completion_mask.float()
            local_rows.extend([row for row in rows])
            offset += chunk_size
        if offset != len(inputs):
            raise ValueError('本地 completion mask 数量与 inputs 数量不一致')
        ...
        if self.scale_rewards in ['group', 'none', 'gdpo']:
            ...
            rewards_std = total_rewards.std().item() if total_rewards.numel() > 1 else 0.0
        if num_generations > 1:
