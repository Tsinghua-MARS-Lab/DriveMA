...
class GRPOArgumentsMixin(RolloutTrainerArgumentsMixin):
    ...
    # 多轮对话：第一轮/第二轮 reward 分别做组内 GRPO 归一化，再按 token 位置写入二维 advantages
    per_turn_grpo_credit: bool = False
    per_turn_loss_weight_turn1: float = 0.5
    per_turn_loss_weight_turn2: float = 0.5
    ...
    def __post_init__(self):
        super().__post_init__()
        if self.per_turn_loss_weight_turn1 < 0 or self.per_turn_loss_weight_turn2 < 0:
            raise ValueError('per_turn_loss_weight_turn1 / per_turn_loss_weight_turn2 必须非负')
        if self.per_turn_loss_weight_turn1 == 0 and self.per_turn_loss_weight_turn2 == 0:
            raise ValueError('per_turn_loss_weight_turn1 和 per_turn_loss_weight_turn2 不能同时为 0')
