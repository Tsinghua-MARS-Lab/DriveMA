...
_TURN1_LAT_ACTIONS = sorted(_MA_LAT_STRAIGHT | _MA_LAT_LEFT | _MA_LAT_RIGHT, key=len, reverse=True)
_TURN1_LAT_ACTIONS_RE = "|".join(re.escape(action) for action in _TURN1_LAT_ACTIONS)
_TURN1_META_ACTION_RE = re.compile(
    rf"lon1\s*:\s*[^,\n;]+"
    rf"\s*[,;]\s*lon2\s*:\s*[^,\n;]+"
    rf"\s*[,;]\s*lat\s*:\s*(?:{_TURN1_LAT_ACTIONS_RE})",
    re.IGNORECASE,
)
...
class MultiTurnScheduler(RolloutScheduler, ABC):
    ...
    async def run(self, infer_request: 'RolloutInferRequest', request_config: 'RequestConfig',
        ...
        while True:
            ...
            self._truncate_turn1_completion(response_choice, current_turn)
    ...
    def _truncate_turn1_completion(
        self,
        ...
        match = _TURN1_META_ACTION_RE.search(completion)
        if match is None:
            return

        truncated_completion = completion[:match.end()].rstrip()
        if truncated_completion == completion:
            return

        response_choice.message.content = truncated_completion

        token_ids = list(response_choice.token_ids or [])
        tokenizer = self.tokenizer
        if tokenizer is not None:
            truncated_token_ids = tokenizer.encode(truncated_completion, add_special_tokens=False)
            response_choice.token_ids = truncated_token_ids
            if response_choice.logprobs is not None and 'content' in response_choice.logprobs:
                if token_ids[:len(truncated_token_ids)] == truncated_token_ids:
                    response_choice.logprobs['content'] = response_choice.logprobs['content'][:len(truncated_token_ids)]
                else:
...
class FixedDialogueMultiTurnScheduler(MultiTurnScheduler):
    """
    多轮对话 GRPO：按数据集中**每个 user 轮**依次生成 assistant，全部轮次完成后再算 reward。

    典型数据为 ``user -> assistant(GT) -> user -> assistant(GT)`` 时，默认流程只会去掉最后一个
    assistant，第一轮仍保留标注，模型只补全最后一轮。本调度器在预处理阶段去掉**所有** assistant，
    保留 ``system``（若有）与第一个 ``user`` 作为首轮输入；后续 ``user`` 消息保存在
    ``data_dict['_grpo_pending_user_messages']``，在每轮 ``step`` 中依次拼入，从而两轮均由模型生成。

    使用方式：训练命令中设置 ``--multi_turn_scheduler fixed_dialogue``；若需与 CLI 的 ``max_turns``
    同时生效，请保证 ``--max_turns`` 不小于样本中的 user 轮数（或留空 ``max_turns``，仅由数据决定轮数）。

    ``check_finished`` 以 ``data_dict['_grpo_num_dialogue_turns']``（user 条数）为准；中间轮次的
    ``response_token_ids`` 在 ``step`` 中返回，以便与 colocate 多轮 rollout 对齐。
    """
    first_turn_max_response_tokens = 100

    def preprocess_rollout_messages(self, input_item: Dict[str, Any]) -> None:
        messages = input_item['messages']
        data_dict = input_item.setdefault('data_dict', {})
        # 供 per_turn_grpo_credit：若样本中尚未提供 gt_assistant_turns，则保存标注中的两轮 assistant 文本
        gt_texts = [
            m['content'] for m in messages
            if m.get('role') == 'assistant' and isinstance(m.get('content'), str)
        ]
        if len(gt_texts) >= 2 and 'gt_assistant_turns' not in data_dict:
            data_dict['gt_assistant_turns'] = gt_texts[:2]
        stripped = [m for m in messages if m.get('role') != 'assistant']
        user_indices = [i for i, m in enumerate(stripped) if m.get('role') == 'user']
        if len(user_indices) < 2:
            raise ValueError(
                'fixed_dialogue 需要至少 2 轮 user（去掉 assistant 后）。'
                '请使用 user/assistant/user/assistant 形式的多轮数据。'
                f' 当前 user 条数={len(user_indices)}')

        first_user_idx = user_indices[0]
        initial = stripped[:first_user_idx + 1]
        pending = [stripped[i] for i in user_indices[1:]]
        input_item['messages'] = initial
        data_dict['_grpo_pending_user_messages'] = pending
        data_dict['_grpo_num_dialogue_turns'] = len(user_indices)

    def check_finished(self, infer_request: 'RolloutInferRequest', response_choice: 'ChatCompletionResponseChoice',
                       current_turn: int) -> bool:
        n = infer_request.data_dict.get('_grpo_num_dialogue_turns')
        if n is not None:
            return current_turn >= n
        if response_choice.finish_reason == 'length':
            return True
        return super().check_finished(infer_request, response_choice, current_turn)
    ...
    def step(self, infer_request: 'RolloutInferRequest', response_choice: 'ChatCompletionResponseChoice',
             current_turn: int) -> Dict:
        pending = infer_request.data_dict.get('_grpo_pending_user_messages') or []
        token_ids = list(response_choice.token_ids) if response_choice.token_ids else []
        rollout_logprobs = self._extract_logprobs_from_choice(response_choice)
        if current_turn == 1 and pending and len(token_ids) > self.first_turn_max_response_tokens:
            token_ids = token_ids[:self.first_turn_max_response_tokens]
            if rollout_logprobs:
                rollout_logprobs = rollout_logprobs[:self.first_turn_max_response_tokens]
            if self.tokenizer is not None and infer_request.messages and infer_request.messages[-1]['role'] == 'assistant':
                infer_request.messages[-1]['content'] = self.tokenizer.decode(
                    token_ids, skip_special_tokens=False)
        ret: Dict[str, Any] = {'infer_request': infer_request}
        if token_ids:
            ret['response_token_ids'] = token_ids
        if rollout_logprobs:
            ret['rollout_logprobs'] = rollout_logprobs
        if not pending:
            return ret
        user_msg = pending.pop(0)
        infer_request.messages.append(user_msg)
        return ret
...
multi_turns = {
    ...
    'fixed_dialogue': FixedDialogueMultiTurnScheduler,
}
