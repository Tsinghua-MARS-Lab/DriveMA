...
1) 默认：<decision> 与 <answer> 轨迹；或 infer 多行 JSONL（generation / generation_turn1+2），
   ...
   1Hz 四点轨迹在 t=0 补零后线性插值为 2Hz（0.5s..4s）
   → 按 trajectory_reward_navsim.py 的 navsim 规则推断纵向 / 横向动作组
...
# ---------------------------------------------------------------------------
# 以下与 examples/train/grpo/plugin/trajectory_reward_navsim.py 同源，避免 import swift
# ---------------------------------------------------------------------------
...
def _cluster_apply_longitudinal_meta_str(future_traj: List[List[float]]) -> str:
    """
    与 trajectory_reward_navsim.py 对齐：读取聚类 report 的 suggested_rule，
...
def _meta_refine_straight_lateral_action_list(
    ...
    """
    同步 trajectory_reward_navsim.py 的直行组规则：只有末端相对首点横向位移超过阈值时，
...
def infer_meta_action_lists_from_future_traj_reward(
    ...
    """
    与 trajectory_reward_navsim.py 完全对齐：
    纵向读取 navsim/train_thresholds_report_4s.json 的 suggested_rule；
    横向使用同一组阈值与直行组 refine 规则。
    """
    if not future_traj or len(future_traj) < 2:
...
def _interp_1hz_4s_traj_to_2hz_4s(traj_1hz_xy: np.ndarray) -> List[List[float]]:
    """
    与 trajectory_reward_navsim.py 对齐：模型输出为 1Hz、对应 1s..4s 的 4 个未来点；
    在 t=0 处补 (0,0) 后线性插值得到 0.5s..4.0s 的 8 个 2Hz 点。
    """
...
def main() -> None:
    ...
    parser.add_argument(
        "--infer_jsonl",
        type=str,
        default='/cephfs/zhengwc/FluidDrive/ms-swift-3.5/infer_outputs/Qwen3.5-2B/Mutil-Turn-w-mask-subsample/RL-rfs-20ep/620-best-now/620-test_outputs.jsonl',
        help="推理结果 JSONL，每行含 generation（或 generation_turn1/turn2）；"
        "默认模式需可解析的 longitudinal/lateral 文案 + 四点轨迹；"
