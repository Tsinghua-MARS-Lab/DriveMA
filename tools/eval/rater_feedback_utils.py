...
def get_lat_lng_thresholds(
    init_speed: np.ndarray,  # [B] or [B, P]
    lat_lng_threshold_multipliers: Tuple[float, float],
) -> Tuple[np.ndarray, np.ndarray]:
  """Get lateral and longitudinal thresholds.
  
  Args:
    init_speed: Initial speed, shape [B] or [B, P] where P is number of raters
    lat_lng_threshold_multipliers: (lat_multiplier, lng_multiplier)
    
  Returns:
    lat_thresholds: [B, 2] or [B, P, 2]
    lng_thresholds: [B, 2] or [B, P, 2]
  """
  ...
  lat_thresholds = scale_by_init_speed[..., None] * lat_thresholds  # [B, 2] or [B, P, 2]
  lng_thresholds = scale_by_init_speed[..., None] * lng_thresholds  # [B, 2] or [B, P, 2]
...
def get_rater_feedback_score(
    ...
    init_speed,  # [B] or List[List[float]] (each sample has list of rater speeds)
  ...
  Args:
    ...
    init_speed: Initial velocities. Can be:
      - np.ndarray with shape [B]: same speed for all raters in each sample
      - List[List[float]]: each sample has list of speeds, one per rater
  ...
  # Process init_speed: convert to [B, P] if needed
  if isinstance(init_speed, list):
    # List[List[float]] -> [B, P]
    init_speed_array = np.zeros(
        (len(init_speed), default_num_of_rater_specified_trajectories),
        dtype=np.float32
    )
    for i, speeds in enumerate(init_speed):
      speeds_arr = np.array(speeds, dtype=np.float32)
      # Pad or truncate to P
      if len(speeds_arr) >= default_num_of_rater_specified_trajectories:
        init_speed_array[i] = speeds_arr[:default_num_of_rater_specified_trajectories]
      else:
        init_speed_array[i, :len(speeds_arr)] = speeds_arr
        # Pad with last value if needed
        if len(speeds_arr) > 0:
          init_speed_array[i, len(speeds_arr):] = speeds_arr[-1]
    init_speed = init_speed_array
  elif isinstance(init_speed, np.ndarray):
    if init_speed.ndim == 1:
      # [B] -> [B, P] by broadcasting the same speed to all raters
      init_speed = np.repeat(
          init_speed[:, None], 
          default_num_of_rater_specified_trajectories, 
          axis=1
      )
    # else: already [B, P]
  else:
    raise ValueError(f"Unsupported init_speed type: {type(init_speed)}")
  ...
  # lng_thresholds and lat_thresholds are now [B, P, 2]
  normalized_lng_distances = (
      lng_distances / lng_thresholds[..., None, :]
  )  # [B, P, I, 2] / [B, P, 1, 2] -> [B, P, I, 2]
  normalized_lat_distances = (
      lat_distances / lat_thresholds[..., None, :]
  )  # [B, P, I, 2] / [B, P, 1, 2] -> [B, P, I, 2]
