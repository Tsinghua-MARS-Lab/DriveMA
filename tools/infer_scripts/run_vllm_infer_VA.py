...
def build_conversation_from_meta_sample(sample: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Build VLM conversation format on-the-fly from raw meta jsonl sample fields."""
...
def get_reference_answer(
    ...
    """Build reference answer from meta sample's future_traj field."""
    future_traj = sample.get("future_traj", None)
    if not future_traj or not isinstance(future_traj, list) or len(future_traj) == 0:
        return ""
    try:
        return _build_future_answer(future_traj=future_traj, indices=future_indices)
    except Exception:
        return ""
...
def _prepare_single_sample_inputs(args):
    """Load images and build vllm inputs for a single sample in parallel (IO-bound, suited for thread pool)."""
    sample, chat_tokenizer = args
    images = load_front_images(sample)
    conversation = build_conversation_from_meta_sample(sample)
    ...
    def _worker():
        with ThreadPoolExecutor(max_workers=io_workers) as exe:
            for batch_idx in range(0, len(samples), batch_size):
                ...
                futs = [exe.submit(_prepare_single_sample_inputs, a) for a in task_args]
                vllm_ins, valid_s = [], []
...
def run_inference(
    ...
    """Run inference: read raw samples from meta jsonl, build prompts on-the-fly, output jsonl."""
    ...
    with open(output_jsonl, file_mode, encoding="utf-8") as outfile:
        with tqdm(total=total_batches, desc="Inference progress") as pbar:
            while True:
                ...
                for (sample, repeat_idx), output in zip(valid_meta, outputs):
                    generation_text = output.outputs[0].text.strip()
                    sample_id = sample.get("sample_id", "")
                    result = {
                        "index": total_processed,
                        "sample_id": sample_id,
                        "generation": generation_text,
                        "reference": get_reference_answer(sample),
                    }
                    if repeat_num > 1:
                ...
                print(f"[INFO] Batch {batch_idx // batch_size + 1}/{total_batches} done, "
                      ...
                      f"inference={_t_infer_end - _t_infer_start:.1f}s")
...
def main():
    parser = argparse.ArgumentParser(
        description="VLM inference: read raw trajectory data from meta jsonl, build prompts on-the-fly, output inference results jsonl"
    )
    ...
    parser.add_argument(
        "--output_jsonl",
        type=str,
        default='/cephfs/zhengwc/FluidDrive/ms-swift-3.5/output/Qwen3.5-2B/Mutil-Turn-w-mask-subsample/VA/v0-20260523-165454/checkpoint-1198/test_outputs_715_scenes_for_vis.jsonl',
        help="Output JSONL file path (each line contains index, generation, reference)",
    )
    parser.add_argument(
        "--model_path",
        type=str,
        default='/cephfs/zhengwc/FluidDrive/ms-swift-3.5/output/Qwen3.5-2B/Mutil-Turn-w-mask-subsample/VA/v0-20260523-165454/checkpoint-1198',
        help="VLM model path",
    )
    parser.add_argument(
        "--batch_size",
