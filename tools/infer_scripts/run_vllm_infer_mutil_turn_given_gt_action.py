...
def _build_answer_only_ref(
    ...
    return "<answer> " + ", ".join(pts) + " </answer>"


def _decision_inner_from_label_row(obj: Dict[str, Any]) -> str:
    """Same convention as training COT: optional explicit ``decision``, else lon/lat line."""
    d = obj.get("decision")
    if isinstance(d, str) and d.strip():
        return d.strip()
    lon = obj.get("longitudinal_action")
    lat = obj.get("lateral_action")
    lon_s = lon.strip() if isinstance(lon, str) else (str(lon).strip() if lon is not None else "")
    lat_s = lat.strip() if isinstance(lat, str) else (str(lat).strip() if lat is not None else "")
    if lon_s or lat_s:
        return f"longitudinal action: {lon_s}; lateral action: {lat_s}"
    return ""


def gt_assistant_turn1_from_label_row(obj: Dict[str, Any]) -> str:
    """First-turn assistant text: <decision> ... </decision> (GT from meta_action jsonl)."""
    inner = _decision_inner_from_label_row(obj)
    return f"{inner}"
...
def load_meta_action_label_map_jsonl(path: str) -> Dict[str, Dict[str, Any]]:
    """Load meta_action_label jsonl -> sample_id -> row dict."""
    ...
    with open(path, "r", encoding="utf-8") as f:
        for line_no, line in enumerate(f, start=1):
            ...
            try:
                ...
                print(f"[WARN] meta_action_label jsonl line {line_no}: {e}, skip")
                continue
...
def _start_prefetch_loader(
    ...
    repeat_num,
    chat_tokenizer,
    meta_action_map: Dict[str, Dict[str, Any]],
    io_workers: int = 16,
):
    """Prefetch vLLM inputs for turn 2 only: GT <decision> from meta_action_label_jsonl, then <answer> gen."""
        ...
        with ThreadPoolExecutor(max_workers=io_workers) as exe:
            for batch_idx in range(0, len(samples), batch_size):
                ...
                for s in batch:
                    ...
                    row = meta_action_map.get(sid_key)
                    if row is None:
                        print(f"[WARN] no meta_action_label for sample_id={sid_key!r}, skip")
                        continue
                    if not _decision_inner_from_label_row(row):
                        print(
                            f"[WARN] empty decision/lon/lat in meta_action_label for sample_id={sid_key!r}, skip"
                        )
                        continue
                    pending.append(s)
                ...
                task_args = [
                    (
                        s,
                        gt_assistant_turn1_from_label_row(meta_action_map[str(s.get("sample_id", "")).strip()]),
                        chat_tokenizer,
                    )
                    for s in pending
                ]
                futs = [exe.submit(_prepare_turn2_sample_inputs, a) for a in task_args]
                vllm_ins, valid_s = [], []
...
def run_inference(
    ...
    """Turn 1 uses GT <decision> from meta_action_label_jsonl; only turn 2 is model-generated (<answer>)."""
    print(f"[INFO] Starting inference on GPU {gpu_start_id}-{gpu_start_id + tensor_parallel_size - 1}")

    print(f"[INFO] Loading meta_action labels: {meta_action_label_jsonl}")
    meta_action_map = load_meta_action_label_map_jsonl(meta_action_label_jsonl)
    print(f"[INFO] Loaded {len(meta_action_map)} meta_action rows (by sample_id)")
    ...
    with open(output_jsonl, file_mode, encoding="utf-8") as outfile:
        with tqdm(total=total_batches, desc="Inference progress") as pbar:
            while True:
                ...
                for (sample, repeat_idx), o2 in zip(valid_meta, outputs_turn2):
                    sample_id = sample.get("sample_id", "")
                    sid_key = str(sample_id).strip()
                    row = meta_action_map.get(sid_key, {})
                    generation_turn1 = gt_assistant_turn1_from_label_row(row)
                    ...
                    result = {
                        ...
                        "generation": combined,
                        "reference_turn1": generation_turn1,
                ...
                print(f"[INFO] Batch {batch_idx // batch_size + 1}/{total_batches} done, "
                      ...
                      f"inference_turn2_only={_t_infer_end - _t_infer_start:.1f}s")
...
def run_multi_gpu_shard_inference(
    ...
    *,
    ...
    """Split samples into num_shards JSONLs, run num_shards worker subprocesses, merge outputs."""
    if num_shards < 2:
...
def main():
    parser = argparse.ArgumentParser(
        description="Turn2-only inference: GT <decision> from --meta_action_label_jsonl, model generates <answer>."
    )
    ...
    parser.add_argument(
        "--meta_action_label_jsonl",
        type=str,
        default="/cephfs/zhengwc/LongTail_synthesizer_V2/v2/data/Meta-Action/val_samples_479_meta_action_label.jsonl",
        help="JSONL with sample_id, longitudinal_action, lateral_action (and optional decision) for GT first turn",
    )
    parser.add_argument(
        "--input_jsonl",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Output JSONL: generation_turn1=GT decision, generation_turn2=model, reference_turn2=<answer> GT",
    )
    parser.add_argument(
        "--model_path",
        type=str,
        default='/cephfs/zhengwc/FluidDrive/ms-swift-3.5/output/Qwen3.5-2B/Mutil-Turn-w-mask-subsample/RL-rfs-20ep/v0-20260415-171333/checkpoint-620',
        help="VLM model path",
    )
    parser.add_argument(
        "--batch_size",
    ...
    parser.add_argument(
        ...
        "merge to --output_jsonl. Use 1 for single-process.",
    )
    parser.add_argument(
        "--gpu_start_id",
        type=int,
    ...
    if not os.path.isfile(args.meta_action_label_jsonl):
        raise SystemExit(f"--meta_action_label_jsonl not found: {args.meta_action_label_jsonl}")
    ...
    if args.num_shards > 1:
        ...
        run_multi_gpu_shard_inference(
            ...
            meta_action_label_jsonl=args.meta_action_label_jsonl,
