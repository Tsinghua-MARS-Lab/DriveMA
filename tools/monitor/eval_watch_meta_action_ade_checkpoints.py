...
run_vllm_infer_mutil_turn.py → cal_meta_action_acc.py + cal_ade.py，
同时统计 meta action 准确率与 3s/5s ADE；**选 best 默认按 5s ADE（越小越好）**，
full_metrics 内保留两套指标。

典型用法（训练每 100 step 存一次，持续往同一 checkpoint_root 写）：

  python eval_watch_meta_action_ade_checkpoints.py \\
...
- best 写在 output_dir/best_model.json（含 meta_action + ade）；历史追加在 output_dir/history.jsonl。
- 默认在评测完成后删除非 best 的 checkpoint-* 目录（出现新 best 时删除旧 best）；每轮扫描末也会按 state+best 清理已评测非 best。可用 --no_delete_non_best 关闭。
"""
...
DEFAULT_INFER = "/cephfs/zhengwc/FluidDrive/ms-swift-3.5/tools/infer_scripts/run_vllm_infer_mutil_turn.py"
DEFAULT_CAL_META = "/cephfs/zhengwc/FluidDrive/ms-swift-3.5/tools/eval-one-step/cal_meta_action_acc.py"
DEFAULT_CAL_ADE = "/cephfs/zhengwc/FluidDrive/ms-swift-3.5/tools/eval-one-step/cal_ade.py"
ROOT= "/cephfs/zhengwc/FluidDrive/ms-swift-3.5/output/RL-exp/Qwen3.5-2B/RL-full/v4-20260513-143414"
...
def run_cal_ade(
    *,
    ...
    if scenario_json:
        cmd.extend(["--scenario_json", scenario_json])
    print(f"[eval_watch] 启动 ADE: {' '.join(cmd)}", flush=True)
    subprocess.run(cmd, check=True)
    with open(metrics_out, "r", encoding="utf-8") as f:
        return json.load(f)
...
def pick_metric_from_merged(merged: Dict[str, Any], key: str) -> Optional[float]:
    """从合并后的 metrics 中取标量：先查 meta_action，再查 ade。"""
...
def update_best_file(
    ...
    *,
    ...
    """若当前指标优于历史 best 则写 best 文件。ADE 类为越小越好，acc 类为越大越好。"""
    new_val = pick_metric_from_merged(stats_merged, best_metric_key)
    if new_val is None:
        return False, None, None

    lower_better = is_lower_better_metric(best_metric_key)
    ...
    payload = {
        ...
        "best_value": new_val,
        "best_is_lower_better": lower_better,
...
def process_one_checkpoint(
    *,
    ...
    metrics_ade_out = output_dir / "metrics" / f"{infer_prefix}{name}_ade.json"
    ...
    stats_ade = run_cal_ade(
        python_exe=python_exe,
        ade_script=cal_ade_script,
        infer_jsonl=str(infer_out),
        metrics_out=str(metrics_ade_out),
        scenario_json=ade_scenario_json,
    )

    stats_merged = {"meta_action": stats_meta, "ade": stats_ade}
    ...
    record = CheckpointRecord(
        ...
        metrics_ade_json=str(metrics_ade_out.resolve()),
    ...
    hist_row = {
        ...
        "best_metric": best_metric_key,
        "best_is_lower_better": is_lower_better_metric(best_metric_key),
        "is_new_best": updated,
        "previous_best": old_b,
    ...
    lb = is_lower_better_metric(best_metric_key)
    if updated:
        cmp_word = "低于" if lb else "高于"
        print(
            f"[eval_watch] ★ 新 best ({best_metric_key}): {new_b} "
            f"({cmp_word}上次 best={old_b}, checkpoint={name})",
            flush=True,
        )
    else:
        print(
            f"[eval_watch] 完成 {name}: {best_metric_key}={new_b} "
            f"(当前历史 best={old_b}) | joint={record.joint_acc} ade_5s={record.ade_5s}",
            flush=True,
        )
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="监控 checkpoint-*：mutil_turn 推理 + meta action acc + ADE；默认按 ade_5s 选 best"
    )
    p.add_argument(
        "--checkpoint_root",
        type=str,
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="可选：传给 cal_ade --scenario_json，仅对指定 sample_id 子集算 ADE",
    )
    p.add_argument(
        "--python",
        type=str,
    ...
    p.add_argument(
        ...
        type=str,
        default="ade_5s",
        choices=("joint_acc", "longitudinal_acc", "lateral_acc", "ade_3s", "ade_5s"),
        help="选 best 的主指标：ade_5s/ade_3s 越小越好，acc 类越大越好",
    )
    p.add_argument(
        "--no_delete_non_best",
        action="store_true",
    ...
    cal_ade_script = Path(args.cal_ade_script).resolve()
    ...
    if not cal_ade_script.is_file():
        raise SystemExit(f"ADE 脚本不存在: {cal_ade_script}")
